...
class FareRemoteDataSourceImpl implements FareRemoteDataSource {
  ...
  Future<List<StopModel>> getConnectedStops(String originId) async {
    final stopwatch = Stopwatch()..start();
    developer.log('🔼 Req: [getConnectedStops] originId: "$originId"', name: 'Supabase');
    ...
    try {
      ...
      final results = (response as List).map((json) => StopModel(
        id: json['destination_id'],
        nameBn: json['destination_name'],
      )).toList();

      stopwatch.stop();
      developer.log('✅ Res: [getConnectedStops] (${stopwatch.elapsedMilliseconds}ms) returned ${results.length} stops', name: 'Supabase');

      return results;
    } catch (e, stackTrace) {
      stopwatch.stop();
      developer.log('❌ Err: [getConnectedStops] (${stopwatch.elapsedMilliseconds}ms)', name: 'Supabase', error: e, stackTrace: stackTrace);
      rethrow;
    }
  }
  ...
  Future<List<FareResultModel>> getFares(String originId, String destinationId) async {
    final stopwatch = Stopwatch()..start();
    developer.log('🔼 Req: [getFares] from: $originId to: $destinationId', name: 'Supabase');
    ...
    try {
      ...
      stopwatch.stop();
      developer.log('✅ Res: [getFares] (${stopwatch.elapsedMilliseconds}ms) found ${results.length} results', name: 'Supabase');
